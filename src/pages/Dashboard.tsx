...
export default function Dashboard() {
    ...
    const isExpired = useMemo(() => sub?.access.kind === 'EXPIRED', [sub]);
                ...
                <div className="grid gap-6 md:grid-cols-2 items-start">
                    ...
                    <div className="grid gap-6">
                        <div className="rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
                            <div className="text-xs font-bold uppercase tracking-widest text-slate-500">Membership</div>
                            ...
                            {loading ? (
                                ...
                                <div className="mt-4">
                                    <div className="text-lg font-extrabold">
                                        {sub.access.kind === 'TRIAL' ? 'Free trial' : 'Membership'}
                                    </div>
                                    ...
                                    {sub.access.kind === 'EXPIRED' ? (
                                        <div className="mt-4 rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
                                            ...
                                            <Link to="/renew" className="font-semibold underline underline-offset-4">
                                                Renew now
                                            </Link>
                                        </div>
                                    ) : (
                                        <div className="mt-4 grid gap-2 text-sm text-slate-300">
                                            <div>
                                                Days left:{' '}
                                                <span className="font-bold text-slate-100">{sub.access.daysLeft}</span>
                                            </div>
                                            {sub.access.endsAt ? (
                                            ...
                                            <div className="text-xs text-slate-500">
                                                Plan:{' '}
                                                {sub.subscriptionPlan ?? (sub.access.kind === 'TRIAL' ? 'Trial' : '—')}
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
                            ...
                            <div className="mt-3 gap-3 grid">
                                ...
                                <Link
                                    to="/venues/apply"
                                    className="inline-flex w-full text-center items-center justify-center rounded-xl border border-slate-700 px-5 py-3 text-sm font-semibold text-slate-100 transition hover:bg-slate-800/50">
                                    Apply as a venue
                                </Link>
                            </div>
                        </div>
